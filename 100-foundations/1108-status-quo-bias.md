...
The mechanism: the current arrangement is the reference point (see 1105 — Anchoring). Changing from it requires accepting certain transition costs in exchange for uncertain future benefits. Loss aversion (1106) makes certain costs feel heavier than uncertain gains. The result is a systematic preference for the existing state that is independent of whether it is better or worse than the alternative.
...
**What Next:** Read 1109 (Cognitive Dissonance) — when users have adopted something despite status quo resistance, cognitive dissonance explains why they may rationalize the new state as what they wanted all along. Read 1110 (Dark Patterns) for how status quo bias is exploited through dark patterns that make the current state feel like the only safe option.
